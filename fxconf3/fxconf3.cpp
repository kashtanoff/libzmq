...
public:
	...
	bool signal()
	{
		if (curdil->type) //Продажи
		{
			if (indicator2 == 0)
				return false;
			_step = *indicator2 * 0.35;
			_takeprofit = *indicator2 * 0.55;
		}
		else //Покупки
		{
			if (indicator == 0)
				return false;
			_step = *indicator * 0.35;
			_takeprofit = *indicator * 0.55;
		}
		return true;
	}
	...
	bool close_profit()
	{
		if (curdil->level > index && curdil->basket_weight(curdil->mpc)>0)// first->profit > 0)
		{
			//msg << "I have profit!" << msg_box;
			if (curdil->type == OP_SELL && *indicator < 0)  //Текущая операция sell
			{
				//msg << "SELL must be closed" << msg_box;
				*o_ticket = curdil->orders[index]->ticket;
				*o_lots = curdil->orders[index]->lots;
				*o_openprice = curdil->mpc;
				index++;
				return true;
			}
			if (curdil->type == OP_BUY && *indicator > 0)  //Текущая операция buy
			{
