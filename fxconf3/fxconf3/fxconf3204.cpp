...
public:
	...
	bool calc_next()
	{
		...
		*o_lots = (nextProfit * base_lot - curdil->basket_weight(*o_tpprice, curdil->cur_av_lvl)) / _takeprofit;
		if (_safe_copy)
	...
	bool signal()
	{
		...
		*indicator2 = 0;
		...
		if (curdil->type) //Продажи
		{
			if (curdil->step_peak)
			{
				if (curdil->step_peak - curdil->mpo >= 0.0005)
				{
					curdil->step_peak = 0;
					*indicator2 = 1;
		...
		else //Покупки
		{
			if (curdil->step_peak)
			{
				if (curdil->mpo - curdil->step_peak >= 0.0005)
				{
					curdil->step_peak = 0;
					*indicator = 1;
					bp = true;
	...
	void calc_indicator()
	{
		int i;
		int j, k;
		//int limit = min(max_bars-1, max_bars - counted + _period);
		int end = _period * 2 + 1;
		double FullLength = 2 * _period + 1;
		closes[end] = dillers[0]->mpc;
		highs[end] = max(dillers[0]->mpc, highs[end]);
		lows[end] = min(dillers[0]->mpc, lows[end]);
		tmup = 0;
		tmdown = 1000000;
		//for(i = _period; i >= 0; i--)
		//{
			double sum  = (_period + 1) * closes[end - i];
			double sumw = (_period + 1);
			for(j=1, k=_period; j<=_period; j++, k--)
			{
				sum  += k*closes[end - (i+j)];
				sumw += k;
			...
			tmBuffer[i] = sum/sumw;
			tmup = max(tmBuffer[i], tmup);
			tmdown = min(tmBuffer[i], tmdown);


			double diffup = highs[end - i]-tmBuffer[i];
			double diffdn = lows[end - i]-tmBuffer[i];
			/*if (i == _period && )
			{
				if (diff>=0)
				   {
			...
			wuBuffer[i] = (wuBuffer[i+1]*(FullLength-1)+diffup * diffup)/FullLength;
			wdBuffer[i] = (wdBuffer[i+1]*(FullLength-1)+diffdn * diffdn)/FullLength;
			
			//bp = true;
			//msg << prices[end - i] << " - " << tmBuffer[i] << "\r\n";
		//}
		up_ind = tmBuffer[0] + (_deviation + dillers[1]->level * _delta) * sqrt(wuBuffer[0]);
		dn_ind = tmBuffer[0] - (_deviation + dillers[0]->level * _delta) * sqrt(wdBuffer[0]);
		//*indicator2 = up_ind;
		//msg << msg_box;
	}

	void refresh_prices(double *_closes, double *_highs, double *_lows)
	{
		closes = _closes;
		highs = _highs;
		lows = _lows;
		wuBuffer[_period + 1] = wuBuffer[_period];
		wdBuffer[_period + 1] = wdBuffer[_period];
		if (_new_bar)
			calc_indicator();
	}
	bool close_profit()
...
_DLLAPI void __stdcall c_refresh_prices(int h, double *_closes, double *_highs, double *_lows)
{
	pool[h]->refresh_prices(_closes, _highs, _lows);
}
