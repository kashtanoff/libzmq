...
namespace fxc {
	...
	namespace strategy {
		...
		public:
			double closedDD[2];   //Закрытая просадка для разгрузки
			double prevprice[2];
			ChartData*		  rates;
			double baseProfit;
		...
		protected:
			...
			inline void moveTP() {
				MARK_FUNC_IN
					...
					//Отрабатываем оппозитное усреднение
...
#pragma region Сигналы и фильтры
			...
			inline bool fastma_channel2() {
				return (curdil->type) ?
					fastma[1]->ma[0] - fastma[1]->ma[1] < channel->up[0] - channel->up[1] && curdil->mpo > fastma[1]->ma[0] : //Продажи
					fastma[0]->ma[1] - fastma[0]->ma[0] < channel->up[1] - channel->up[0] && curdil->mpo < fastma[0]->ma[0]; //Покупки
			}
