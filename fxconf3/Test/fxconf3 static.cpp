...
public:
	...
	int		_base_pips;			//68 вести расчет прибыли с базового лота а не с начального
	double	_minlot;			//69 минимальный лот
	...
	void calc_first_lot()
	{
		...
		*o_lots = base_lot;
		...
		if ((curdil->type == OP_BUY && *indicator < (_rsi_delta - 100)) ||
			(curdil->type == OP_SELL && *indicator > -_rsi_delta))	//Отрабатываем увеличение по стохастику
	...
	bool calc_next()
	{
		...
		*o_openprice  = curdil->sl(curdil->last->openprice, nextStep);
		...
		else 
		{
			...
			else if (d <= 0)  //Выставляем по рынку
			{
				*o_openprice = curdil->mpo;
				*o_type = curdil->type;
			}
		...
		*o_tpprice = curdil->tp(*o_openprice, nextTP);
		...
		if (_tr_stop)
		{
			...
			//*o_lots = max(*o_lots, (_takeprofit * _minlot - curdil->order_weight(curdil->first->openprice, *o_tpprice, _minlot)) / nextTP);  //Лот должен покрыть минимальную часть лота первого ордера
	...
	int getjob()
	{
		...
		while(true)		//Чтобы лишний раз не выходить в MQL
		{
			...
			case 0: //Старт нового тика
				...
				{
					if (!_stop_new[curdil->type])	//Если не запрещено открытие новой сетки
						{k=1; 
						if (*indicator==10)
							return(5); //Нужны данные индикатора
						break;
						}	
					k=2; break;
				}
				...
				{
					//bp = true;
					if (_tr_step && curdil->level < curdil->prev_lvl)  //Если чтото закрылось, то сбрасываем пик трейлинг степа
						curdil->step_peak = curdil->mpo;
					if (_tr_stop && curdil->level > curdil->prev_lvl)  //Если чтото открылось, то сбрасываем пик трейлинг стопа
						curdil->tp_peak = 0;
					if (curdil->opposite->level >= _forward_lvl && !curdil->ord_stop)   //Если разрешены форварды и его можно поставить
						{k=5; 
						if (*indicator==10)
							return(5); //Нужны данные индикатора
						break;
					...
					k=3;
				break;
			case 1: //Открытие первого ордера: Получены данные индикатора
				if (!signal())
				{
					k = 2;
					break;
			...
			case 3:	//удаление своего стоп ордера
				...
				break;
			case 4: //удаление лимитного ордера
				k=9;
				if (curdil->ord_limit)
				{
					*o_ticket = curdil->ord_limit->ticket;
					ShowInfo("delorder limit", *o_ticket);
					return(3);	//Удалить лимитый ордер
				}
				break;
			case 5:	//Открытие форварда
				k=6;
				if (calc_forward())
				...
				break;
			case 6:
				if (!curdil->ord_limit)	//Если нет лимитников
					{k=7; break;}  //Двигаем тейкпрофиты
				else if (curdil->prev_lvl > curdil->level)   //Если частичный сброс сетки и есть отложки
					{k = 4; break;}		//удалить лимитники
				k = 9; break;
			case 7:  //двигает тейкпрофиты, открывает последующие ордера
				if (!_tr_stop && move_tp())	//Если есть что двигать, двигаем
					return(2);
				k=9;
				if (!signal())
					break;
				if (calc_next()) //Если можно выставить отложку или по рынку, выставляем
				{
					//bp = true;
					ShowInfo("neworder next", *o_lots);
					return(1);}
				break;
			case 8:  //Открывает первый ордер
				k=2;
				if (calc_first())
				{
				...
				break;
			case 9:  //Проверяем срабатывание виртуального (трейлингового) тейкпрофита
				if (trailing_stop())
					{k = 10; break; }
				k = 100; 
				break;
			case 10:  //Закрыть все что можно в профит
				if (close_profit())
				{
...
_DLLAPI void __stdcall c_setint(int h, int index, int value)
{
	__try
	{
		switch(index)
		{
			...
			case 68:	pool[h]->_base_pips = value;			//68 вести расчет прибыли с базового лота а не с начального
...
_DLLAPI void __stdcall c_setdouble(int h, int index, double value)
{
	__try
	{
		switch(index)
		{
			...
			case 69:	pool[h]->_minlot = value; break;			//69 минимальный лот
